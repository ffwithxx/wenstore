...
#define kCellName @"RecipientsDetailCell"
@interface RecipientsDetailVC ()<RemarkDelegate,UITableViewDelegate,UITableViewDataSource> {
    RecipientsDetailCell *_cell;
    NSInteger lpdt043;
    NSInteger lpdt042;
    NSInteger lpdt036;
...
@implementation RecipientsDetailVC
...
-(void)update{
    ...
    [updateDict setObject:updateDict forKey:@"detail"];
    ...
    if ([self.typeStr isEqualToString:@"add"]) {
        ...
        if (isConfirm == true) {
            urlStr = @"App/Wbp3013/UpdateAndConfirm";
        }else {
            urlStr = @"App/Wbp3013/Update";
        }
    }
    ...
    [[AFClient shareInstance] Update:updateDict withArr:postOneArr withUrl:urlStr  progressBlock:^(NSProgress *progress) {
        ...
        if ([[responseBody valueForKey:@"status"] integerValue] == 200) {
            ...
            if ([BGControl isNULLOfString:[userResponseDict valueForKey:@"code"]] ) {
                ...
                RecipientsOnePageVC *orderVC = [storyboard instantiateViewControllerWithIdentifier:@"RecipientsOnePageVC"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (!_cell) {
        _cell = [[RecipientsDetailCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:kCellName];
    }
    ...
    RecipientsModel *model = self.dataArray[indexPath.section];
