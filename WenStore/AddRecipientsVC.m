...
#import "AddRecipientsCell.h"
#import "RecipientsModel.h"
#import "AFClient.h"
#import "BGControl.h"
#define kCellName @"AddRecipientsCell"
#import "SearchVC.h"
#import "RecipientsDetailVC.h"
#import "orderCountThree.h"
@interface AddRecipientsVC ()<UITableViewDelegate,UITableViewDataSource,fanDataDelegate,asideDelegate,orderCountDelegate,TanDelegate,RecipientsCountDelegate> {
    NSInteger lpdt036;
...
@implementation AddRecipientsVC
...
- (void)first {
    ...
    lpdt036 = [[NSString stringWithFormat:@"%@",[[NSUserDefaults standardUserDefaults] valueForKey:@"3013lpdt036"] ] integerValue];
      lpdt042 = [[NSString stringWithFormat:@"%@",[[NSUserDefaults standardUserDefaults] valueForKey:@"3013lpdt042"] ] integerValue];
    lpdt043 = [[NSString stringWithFormat:@"%@",[[NSUserDefaults standardUserDefaults] valueForKey:@"3013lpdt043"] ] integerValue];
...
-(void)firstOne {
    ...
    if ([self.typeStr isEqualToString:@"add"]) {
        urlStr = @"App/Wbp3013/New";
    }else{
        urlStr = @"App/Wbp3013/Edit";
    }
    [self show];
            ...
            if ([BGControl isNULLOfString:[userResponseDict valueForKey:@"code"]] ) {
                ...
                for (int i = 0; i<dataArr.count; i++) {
                    ...
                    for (int j = 0; j<dictDetail.count; j++) {
                        RecipientsModel *model  = [RecipientsModel new];
...
-(void)jiaoyan {
    ...
    for (int i = 0; i<postArr.count; i++) {
        RecipientsModel *model = postArr[i];
    ...
    [[AFClient shareInstance] postValidateCart:mastDict detail:detailArr uploadImages:uploadImgArr promoOrders:proOrderArr freeOrders:freeOrderArr withArr:postOneArr withUrl:@"App/Wbp3013/ValidateCart" withjsob001:@"" progressBlock:^(NSProgress *progress) {
...
-(void)fanDict:(NSMutableDictionary *)dict
{
    ...
    for (int i = 0; i<self.ruleArr.count; i++) {
        ...
        for (int j = 0; j<arrTwo.count; j++) {
            RecipientsModel *model = arrTwo[j];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (tableView == self.leftTableView){
        ...
        AddRecipientsCell  * _CellTwo = [tableView dequeueReusableCellWithIdentifier:CellIdentifier];
        if (!_CellTwo) {
            _CellTwo = [[AddRecipientsCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:CellIdentifier];
        }
        ...
        RecipientsModel *model = arr[indexPath.row];
...
-(void) getRecipientsOrderCount:(NSDecimalNumber *)count withModel:(RecipientsModel *)model {
    ...
    for (RecipientsModel *oneModel in postArr) {
    ...
    for (int i = 0; i <postArr.count; i++) {
        RecipientsModel *modelTwo = postArr[i];
...
-(void)scrapwithModel:(RecipientsModel *)model{
    ...
    orderView.recipientsModel = model;
    orderView.typeStr = @"3013";
    orderView.recipientsCountDelegate = self;
