...
@implementation CallOrderTwoCell {
...
//    self.remindButton.layer.cornerRadius = 8.f;
...
- (void)showModel:(NewModel*)model withDict:(NSMutableDictionary *)dict widtRight:(NSMutableDictionary *)rightDict  withKey:(NSString *)key withIndex:(NSInteger) currentIndex{
    ...
    self.remindButton.frame =  CGRectMake(self.contentView.frame.size.width -marginWidth-60, Imgwidth+marginWidth*2, 60, 30);
...
//    self.orderCountLab.backgroundColor = kTabBarColor;
    ...
    self.fgView.frame = CGRectMake(marginWidth, Imgwidth+marginWidth*3+shopWidth, self.contentView.frame.size.width-marginWidth*2, 1);
    self.fgView.backgroundColor = kLineColor;
    self.bigView.frame = CGRectMake(0, Imgwidth+marginWidth*3+shopWidth+1, self.contentView.frame.size.width, 16);
...
//    index = currentIndex;
    ...
    if ([BGControl isNULLOfString:model.k1dt003]) {
        ...
        self.specificationsLab.text = [NSString stringWithFormat:@"%@",model.k1dt003];
    }
    ...
    for (int i = 0; i < visiableFieldsArr.count; i++) {
        ...
        if ([visiableFieldsStr isEqualToString:@"K1DT105"]) {
            isNum = true;
        }
...
//    }
    ...
    NSInteger lpdt042 = [[[NSUserDefaults standardUserDefaults] valueForKey:@"lpdt042"] integerValue];;
...
//        self.orderCountLab.frame = CGRectMake(self.contentView.frame.size.width -marginWidth-shopWidth-orderWidth, marginWidth+oneHei*3, orderWidth, shopWidth);
//        self.orderButton.frame = CGRectMake(self.contentView.frame.size.width -marginWidth-shopWidth-orderWidth, marginWidth+oneHei*3, orderWidth, shopWidth);
//        self.jianImg.frame = CGRectMake(self.contentView.frame.size.width-marginWidth-shopWidth*2-orderWidth, marginWidth+oneHei*3, shopWidth, shopWidth);
//        self.minusBth.frame =CGRectMake(self.contentView.frame.size.width-marginWidth-shopWidth*2-orderWidth, marginWidth+oneHei*3, shopWidth, shopWidth);
//    }
...
//    self.bigView.frame = CGRectMake(0, Imgwidth+marginWidth*3+shopWidth+1, self.contentView.frame.size.width, 16);
    ...
    self.xiaView.frame = CGRectMake(0, Imgwidth+marginWidth*3+shopWidth+1, self.contentView.frame.size.width, sumHei);
...
-(void)other:(UIButton *)bth{
    ...
    if (bth.tag == 500) {
        ...
        if (_delegate &&[_delegate respondsToSelector:@selector(getMaxHei:withKey:withIndex:)]) {
            [_delegate getMaxHei:xiaFrame.size.height withKey:keyStr withIndex:oneModel.index];
        }
    }else {
        self.bigView.hidden = NO;
        self.xiaView.hidden = YES;
        if (_delegate &&[_delegate respondsToSelector:@selector(getMaxHei:withKey:withIndex:)]) {
            [_delegate getMaxHei:0 withKey:keyStr withIndex:oneModel.index];
        }
