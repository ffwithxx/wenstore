...
#import "BGControl.h"
@implementation ProcurementOrderBottomCell {
    UIView *bigView;
    UILabel *titleLab;
    UILabel *priceLab;
    UIImageView *jianImg;
...
- (void)showModelWithModel:(AddProcurementModel *)model{
    ...
    if (![[NSString stringWithFormat:@"%@",model.k1dt201Price] isEqualToString:@"0"]) {
        priceLab.text = [NSString stringWithFormat:@"￥%@/%@",[BGControl notRounding:model.k1dt201Price afterPoint:lpdt042],model.k1dt005];
    }else{
        priceLab.text = @"";
    }
   
    orderCountField.text = [BGControl notRounding:model.k1dt101Count afterPoint:lpdt036];;;
...
-(void)buttonClick:(UIButton *)bth {
    ...
    if (bth.tag == 1002) {
        ...
        if (compar != NSOrderedDescending) {
            ...
            NSDecimalNumber*jianfa = [oneModel.k1dt101Count decimalNumberBySubtracting:jiafa1];
            numCount =[NSDecimalNumber decimalNumberWithString: [BGControl notRounding:jianfa afterPoint:lpdt036]];
        }
            }else if (bth.tag == 1004) {
        
    }
    if (_bottomDelegate && [_bottomDelegate respondsToSelector:@selector(procurementWithModel:withTag:withCount:)]) {
        [_bottomDelegate procurementWithModel:oneModel withTag:bth.tag withCount:numCount];
    }
