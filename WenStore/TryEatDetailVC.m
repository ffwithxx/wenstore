...
#define kCellName @"TryEatDetailCell"
@interface TryEatDetailVC ()<RemarkDelegate,UITableViewDelegate,UITableViewDataSource> {
    TryEatDetailCell *_cell;
    NSInteger lpdt043;
    NSInteger lpdt042;
    NSInteger lpdt036;
...
- (void)first {
    ...
    self.tryEatDateField.text = [BGControl changeNsdate:[NSDate date]];
    
    tryEatDate = [NSDate date];
...
-(void)update{
    ...
    [postMast setValue:[NSString stringWithFormat:@"%@",self.tryEatDateField.text] forKey:@"k1mf003"];
    [postMast setValue:[NSString stringWithFormat:@"%@",self.tryEatDateField.text] forKey:@"k1mf004"];
    ...
    if ([self.typeStr isEqualToString:@"add"]) {
        ...
        if (isConfirm == true) {
            urlStr = @"App/Wbp3018/UpdateAndConfirm";
        }else {
            urlStr = @"App/Wbp3018/Update";
        }
    }
    ...
    [[AFClient shareInstance] Update:updateDict withArr:postOneArr withUrl:urlStr  progressBlock:^(NSProgress *progress) {
        ...
        if ([[responseBody valueForKey:@"status"] integerValue] == 200) {
            ...
            if ([BGControl isNULLOfString:[userResponseDict valueForKey:@"code"]] ) {
                ...
                TryEatOnePageVC *orderVC = [storyboard instantiateViewControllerWithIdentifier:@"TryEatOnePageVC"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (!_cell) {
        _cell = [[TryEatDetailCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:kCellName];
    }
    ...
    TryEatModel *model = self.dataArray[indexPath.section];
...
- (IBAction)buttonClick:(UIButton *)sender {
    if (sender.tag == 201) {
        ...
        calendarPicker.calendarBlock = ^(NSInteger day, NSInteger month, NSInteger year){
            ...
            self.tryEatDateField.text = [NSString stringWithFormat:@"%@-%@-%@", yearStr,monthStr,dayStr];
            
            //            NSDate *jiaoDate = [BGControl stringToDate:self.jiaoField.text];
            //            NSString *jiaoSter = [NSString stringWithFormat:@"%@",jiaoDate];
            tryEatDate = [BGControl stringToDate:self.tryEatDateField.text];
        };
