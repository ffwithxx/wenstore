...
#import "ReasonView.h"
@interface AddScrapVC ()<UITableViewDelegate,UITableViewDataSource,orderCountDelegate,fanDataDelegate,TanDelegate,orderCountDelegate,resonTanDelegate,resonDelegate,scrapDelegate,ChoiceresonTanDelegate> {
...
- (void)first {
    ...
    lpdt043 = [[NSString stringWithFormat:@"%@",[[NSUserDefaults standardUserDefaults] valueForKey:@"3002lpdt043"] ] integerValue];
...
-(void)firstOne {
    NSString *urlStr;
    if ([self.typeStr isEqualToString:@"add"]) {
        urlStr = @"App/Wbp3002/New";
    }else {
        urlStr = @"App/Wbp3002/Edit";
    }
    [self show];
            ...
            if ([BGControl isNULLOfString:[userResponseDict valueForKey:@"code"]] ) {
                ...
                for (int i = 0; i<dataArr.count; i++) {
                    ...
                    for (int j = 0; j<dictDetail.count; j++) {
                        AddScrapModel *model  = [AddScrapModel new];
...
//        [modelOne setValue:model.k1dt104 forKey:@"k1dt104"];
    ...
    [self show];
    if ([self.typeStr isEqualToString:@"add"]) {
        [[AFClient shareInstance] postValidateCart:postMastDict detail:detailArr uploadImages:uploadImgArr promoOrders:proOrderArr freeOrders:freeOrderArr withArr:postOneArr withUrl:@"App/Wbp3002/ValidateCart" withjsob001:@"" progressBlock:^(NSProgress *progress) {
        ...
        [[AFClient shareInstance] postValidateCartone:postMastDict detail:detailArr uploadImages:uploadImgArr promoOrders:proOrderArr freeOrders:freeOrderArr withArr:postOneArr withUrl:@"App/Wbp3002/ValidateCart" progressBlock:^(NSProgress *progress) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (tableView == self.leftTableView){
        ...
        AddScrapCell  * _twoCell = [tableView dequeueReusableCellWithIdentifier:CellIdentifier];
        if (!_twoCell) {
            _twoCell = [[AddScrapCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:CellIdentifier];
        }
        ...
        AddScrapModel *model = arr[indexPath.row];
...
- (void)getOrderCount:(NSDecimalNumber *)count withModel:(AddScrapModel *)model {
    ...
    for (AddScrapModel *oneModel in postArr) {
    ...
    for (int i = 0; i <postArr.count; i++) {
        AddScrapModel *modelTwo = postArr[i];
...
-(void)scrapwithModel:(AddScrapModel *)model{
    ...
    orderView.AddScrapModel = model;
    orderView.typeStr = @"3002";
    orderView.scrapDelegate = self;
    ...
    if (self.reasonsArr.count > 0) {
        ...
        for (int i = 0; i < self.reasonsArr.count; i++) {
            UIAlertAction *home1Action = [UIAlertAction actionWithTitle:[self.reasonsArr[i] valueForKey:@"text"] style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) { //TODO:
                [self hilldeAll];
                model.k1dt501 = [self.reasonsArr[i] valueForKey:@"text"];
                model.k1dt502 = [self.reasonsArr[i] valueForKey:@"value"];
                [[self.ruleDita valueForKey:model.keyStr] replaceObjectAtIndex:model.index withObject:model];
                dataDict =[NSMutableDictionary new];
                self.datas = [NSMutableArray array];
                NSInteger countOne = 0;
...
-(void)resonStr:(NSString *)str withKey:(NSString *)key withIndex:(NSInteger)index {
    ...
    AddScrapModel *model = oneArr[index];
    model.k1dt503 = str;
