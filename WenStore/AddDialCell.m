...
- (void)showModel:(DialModel *)model{
    ...
    [self.headImg sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@%@?%@%@&%@%d",picUrl,@"https://fc.winton.com.cn/FileCenter/ProductPicture/ExportMedium",@"productId=",model.k1dt001,@"imge004=",model.imge004]] placeholderImage:[UIImage imageNamed:@"icon_moren(1).png"]];
...
-(void)plusclicks{
    ...
    if (_orderDelegate &&[_orderDelegate respondsToSelector:@selector(getdialOrderCount:withModel:)]) {
        [_orderDelegate getdialOrderCount:[NSDecimalNumber decimalNumberWithString:numStr] withModel:oneModel];
    }
