...
#import "ScrapCell.h"
#import "ScrapModel.h"
#define kCellName @"ScrapCell"
#import "SearchVC.h"
@interface ScrapVC ()<UITableViewDelegate,UITableViewDataSource,fanDataDelegate> {
    NSInteger lpdt036;
...
-(void)first {
    ...
    for (int i = 0; i<dataArr.count; i++) {
        ...
        for (int j = 0; j<dictDetail.count; j++) {
            ScrapModel *model  = [ScrapModel new];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (tableView == self.leftTableView){
        ...
        if (!_twoCell) {
               _twoCell = [[ScrapCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:kCellName];
        ...
        ScrapModel *model = arr[indexPath.row];
