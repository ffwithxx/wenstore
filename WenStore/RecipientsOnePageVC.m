...
#import "RecipientsOnePageCell.h"
#import "BGControl.h"
#import "AFClient.h"
#import <MJRefresh/MJRefresh.h>
#import "MJRefreshComponent.h"
#import "RecipientsOnePageModel.h"
#import "MainViewController.h"
#import "AddRecipientsVC.h"
#import "RecipientsVC.h"
#import "RecipientsDetailVC.h"
#define kCellName @"RecipientsOnePageCell"
@interface RecipientsOnePageVC ()<UITableViewDelegate,UITableViewDataSource,orderDelegete>{
    RecipientsOnePageCell *_cell;
...
@implementation RecipientsOnePageVC
...
-(void)getDate {
    ...
    [self show];
    [[AFClient shareInstance] OnePage:postDict withArr:postOneArr withUrl:@"App/Wbp3013/OnePage" progressBlock:^(NSProgress *progress) {
        ...
        if ([[responseBody valueForKey:@"status"] integerValue]== 200) {
            ...
            if ([BGControl isNULLOfString:[userResponseDict valueForKey:@"code"]] ) {
                ...
                for (int i = 0; i<dataArr.count; i++) {
                    ...
                    RecipientsOnePageModel *model = [RecipientsOnePageModel new];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (!_cell) {
        _cell = [[RecipientsOnePageCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:kCellName];
    }
    ...
    RecipientsOnePageModel *model = self.dataArray[indexPath.section];
...
-(void)postoneStr:(NSString *)tagStr withModel:(RecipientsOnePageModel *)model {
    if ([tagStr isEqualToString:@"301"]) {
        ...
        if (model.isDisplayEditButton) {
            ...
            AddRecipientsVC *mainVC = [storyboard instantiateViewControllerWithIdentifier:@"AddRecipientsVC"];
...
-(void)PreviewWithIdStr:(NSString *)idStr{
    [self show];
    [[AFClient shareInstance] WBP3008Preview:idStr withArr:postOneArr withUrl:@"App/Wbp3013/Preview" progressBlock:^(NSProgress *progress) {
        ...
        if ([[responseBody valueForKey:@"status"] integerValue]==200) {
            ...
            if ([BGControl isNULLOfString:[userResponseDict valueForKey:@"code"]] ) {
                ...
                UIStoryboard *storyboard = [UIStoryboard storyboardWithName:@"WendianTwo" bundle:nil];
                NSDictionary *dict = [responseBody valueForKey:@"data"];
                RecipientsDetailVC *detail = [storyboard instantiateViewControllerWithIdentifier:@"RecipientsDetailVC"];
                detail.idStr = idStr;
                detail.typeStr = @"2";
...
- (void)GreateWith:(NSDictionary *)dataDic{
    ...
    [[AFClient shareInstance] Update:postMast withArr:postOneArr withUrl:@"App/Wbp3013/UpdateAndConfirm"  progressBlock:^(NSProgress *progress) {
...
-(void)delet:(NSString *)idStr {
    [self show];
    [[AFClient shareInstance] Destroy:idStr withArr:postOneArr withUrl:@"App/Wbp3013/Destroy" progressBlock:^(NSProgress *progress) {
    ...
    [[AFClient shareInstance] WBP3008Preview:idStr withArr:postOneArr withUrl:@"App/Wbp3013/Preview" progressBlock:^(NSProgress *progress) {
        ...
        if ([responseBody[@"status"] integerValue] == 200) {
            ...
            if ([BGControl isNULLOfString:[userResponseDict valueForKey:@"code"]] ) {
                ...
                RecipientsVC *detail =[storboard  instantiateViewControllerWithIdentifier:@"RecipientsVC"];
                detail.idStr = idStr;
                detail.dataDict = [responseBody valueForKey:@"data"];
                [self.navigationController pushViewController:detail animated:YES];
