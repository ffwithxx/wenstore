...
#import "SearchVC.h"
#import "BGControl.h"
#define kCellName @"ProducedCell"
@interface ProducedVC ()<UITableViewDelegate,UITableViewDataSource,fanDataDelegate>{
    NSInteger lpdt036;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (tableView == self.leftTableView){
        ...
        // 通过不同标识创建cell实例
        ProducedCell  * _twoCell = [tableView dequeueReusableCellWithIdentifier:kCellName];
        if (!_twoCell) {
            _twoCell = [[ProducedCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:kCellName];
        }
