...
#import "AddGiveAwayCell.h"
#import "GiveAwayModel.h"
#import "AFClient.h"
#import "BGControl.h"
#define kCellName @"AddGiveAwayCell"
#import "SearchVC.h"
#import "GiveAwayDetailVC.h"
#import "orderCountThree.h"
@interface AddGiveAwayVC ()<UITableViewDelegate,UITableViewDataSource,fanDataDelegate,asideDelegate,orderCountDelegate,TanDelegate,giveAwayCountDelegate> {
    NSInteger lpdt036;
...
-(void)firstOne {
    ...
    if ([self.typeStr isEqualToString:@"add"]) {
        urlStr = @"App/Wbp3019/New";
    }else{
        urlStr = @"App/Wbp3019/Edit";
    }
    [self show];
...
-(void)jiaoyan {
    ...
    [[AFClient shareInstance] postValidateCart:mastDict detail:detailArr uploadImages:uploadImgArr promoOrders:proOrderArr freeOrders:freeOrderArr withArr:postOneArr withUrl:@"App/Wbp3019/ValidateCart" withjsob001:@"" progressBlock:^(NSProgress *progress) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (tableView == self.leftTableView){
        ...
        AddGiveAwayCell  * _CellTwo = [tableView dequeueReusableCellWithIdentifier:CellIdentifier];
        if (!_CellTwo) {
            _CellTwo = [[AddGiveAwayCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:CellIdentifier];
        }
...
-(void) getGiveAwayOrderCount:(NSDecimalNumber *)count withModel:(GiveAwayModel *)model {
    ...
    for (GiveAwayModel *oneModel in postArr) {
    ...
    for (int i = 0; i <postArr.count; i++) {
        GiveAwayModel *modelTwo = postArr[i];
...
-(void)scrapwithModel:(GiveAwayModel *)model{
    ...
    orderView.giveAwayModel = model;
    orderView.typeStr = @"3019";
    orderView.giveAwayCountDelegate = self;
