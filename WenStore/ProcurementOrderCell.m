...
@implementation ProcurementOrderCell {
NSInteger lpdt036;
NSInteger lpdt042;
...
//        [self.contentView addSubview:view];
    ...
    self.specificationsLab.frame = CGRectMake(Imgwidth+marginWidth*2, marginWidth+oneHei, self.contentView.frame.size.width - Imgwidth-marginWidth*3, oneHei);
     self.priceLab.frame = CGRectMake(Imgwidth+marginWidth*2, marginWidth+oneHei*2, self.contentView.frame.size.width - Imgwidth-marginWidth*3, oneHei);
...
- (void)buttonClick:(UIButton *)bth {
    if (bth.tag == 1004 || bth.tag == 1007) {
        ...
        if (_procurementDelegate && [_procurementDelegate respondsToSelector:@selector(procurementWithModel:withTag:withCount:)]) {
            [_procurementDelegate procurementWithModel:oneModel withTag:bth.tag withCount:numCount];
        }
