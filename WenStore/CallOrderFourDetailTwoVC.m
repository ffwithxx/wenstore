...
#import "SearchVC.h"
#import "EditModel.h"
#define kCellName @"CallOrderFourDetailTwoCell"

@interface CallOrderFourDetailTwoVC ()<UITableViewDataSource,UITableViewDelegate,fanDataDelegate> {
...
@implementation CallOrderFourDetailTwoVC
...
-(void)firstOne{
    [self show];
    NSString *urlStr = @"App/Wbp3011/Preview";
    if ([self.typeStr isEqualToString:@"2"]) {
       urlStr = @"App/Wbp3011/Approve";
    }
    [[AFClient shareInstance] WBP3008Preview:self.idStr withArr:postOneArr withUrl:urlStr progressBlock:^(NSProgress *progress) {
            ...
            if ([BGControl isNULLOfString:[userResponseDict valueForKey:@"code"]] ) {
                ...
                for (int i = 0; i<dataArr.count; i++) {
                    ...
                    for (int j = 0; j<dictDetail.count; j++) {
                        ...
                        if ([urlStr isEqualToString:@"App/Wbp3011/Approve"]) {
                            if (![[NSString stringWithFormat:@"%@",model.k1dt104] isEqualToString:@"0"]) {
                    ...
                    if (arr.count > 0) {
                        ...
                        [dataDict setObject:arr forKey:[NSString stringWithFormat:@"%ld",(long)sectionNum]];
                        sectionNum ++;
                    }
                }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (tableView == self.leftTableView){
        ...
        // 通过不同标识创建cell实例
             CallOrderFourDetailTwoCell  * _twoCell = [tableView dequeueReusableCellWithIdentifier:kCellName];
        if (!_twoCell) {
           
            _twoCell = [[CallOrderFourDetailTwoCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:kCellName];
