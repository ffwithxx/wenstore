...
#import "FourDetailVC.h"
#import "BGControl.h"
#import "EditModel.h"
#import "FourDetailCell.h"
#define kCellName @"FourDetailCell"
#import "kantuViewController.h"
@interface FourDetailVC ()<UITableViewDelegate,UITableViewDataSource,UIScrollViewDelegate>{
    NSInteger lpdt042;
    NSInteger lpdt043;
    NSInteger lpdt036;
    NSMutableArray *postArr;
    NSMutableArray *postOneArr;
    FourDetailCell *_cell;
...
-(void)first{
    ...
    for (int i = 0; i<dataArr.count; i++) {
        ...
        for (int j = 0; j<dictDetail.count; j++) {
            ...
            if ([self.tagStr isEqualToString:@"301"]) {
                model.orderCount = [dictOne valueForKey:@"k1dt104"];
                ...
                if (![jiheStr isEqualToString:@"0"] && ![BGControl isNULLOfString:jiheStr] &&![jiheStr isEqualToString:@"(null)"]) {
                    count = [count decimalNumberByAdding:model.orderCount];
                    [postArr addObject:model];
                }
            }else {
                if (![[NSString stringWithFormat:@"%@",model.k1dt101] isEqualToString:@"0"] ||![[NSString stringWithFormat:@"%@",model.k1dt103] isEqualToString:@"0"]) {
                    count = [count decimalNumberByAdding:model.orderCount];
                    [postArr addObject:model];
                }
            }
    ...
    self.countLab.text = [NSString stringWithFormat:@"%@%@%@",@"已收", [BGControl notRounding:count afterPoint:lpdt036],@"件商品"];
    [self setdata];
...
- (void)setScrollView {
    ...
    if (uploadImages.count >0) {
        ...
        for (int i = 0; i<uploadImages.count; i++) {
            ...
            [_imageview sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@%@?%@%@",picUrl,@"/FileCenter/StoreAttachment/ExportMedium",@"pict001=",[uploadImages[i] valueForKey:@"systemFileName"]]] placeholderImage:[UIImage imageNamed:@"icon_moren(1).png"]];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (!_cell) {
        _cell = [[FourDetailCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:kCellName];
    }
