...
#define kCellName @"ScrapDetailCell"
@interface ScrapDetailVC ()<RemarkDelegate,UITableViewDelegate,UITableViewDataSource,UIActionSheetDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate,UITextFieldDelegate,UITextViewDelegate,ELCImagePickerControllerDelegate,postArrDelegate,UIScrollViewDelegate> {
    ScrapDetailCell *_cell;
...
-(void)first {
    ...
    self.tuihuoDateFile.text = [BGControl changeNsdate:[NSDate date]];
    tuihuiDate = [NSDate date];
    ...
    self.sumPriceLab.text =[NSString stringWithFormat:@"%@%@",@"总计: ￥", [masterDict valueForKey:@"k1mf302"]];
...
-(void)update{
    ...
    if ([self.typeStr isEqualToString:@"four"]) {
        urlStr = @"App/Wbp3002/UpdateAndConfirm";
    }else if ([self.typeStr isEqualToString:@"fourNew"]){
        if (isConfirm == true) {
            urlStr = @"App/Wbp3002/CreateAndConfirm";
        }else {
            urlStr = @"App/Wbp3002/Create";
        }
    ...
    [[AFClient shareInstance] Update:updateDict withArr:postOneArr withUrl:urlStr  progressBlock:^(NSProgress *progress) {
        ...
        if ([[responseBody valueForKey:@"status"] integerValue] == 200) {
            ...
            if ([BGControl isNULLOfString:[userResponseDict valueForKey:@"code"]] ) {
                ...
                ScrapOnePageVC *callOrderThree = [storyboard instantiateViewControllerWithIdentifier:@"ScrapOnePageVC"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (!_cell) {
        _cell = [[ScrapDetailCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:kCellName];
    }
    ...
    AddScrapModel *model = self.dataArray[indexPath.section];
...
- (void)creatImgScr {
    ...
    if (_images.count >0) {
        ...
        [self.bigScrollView setContentSize:CGSizeMake(_images.count*60+(_images.count-1)*5 +65, workingFrame.size.height)];
        BJView.frame = CGRectMake(0, 0, _images.count*60+(_images.count-1)*5, 80);
    }
...
-(void)uploadPicturewithInsdex:(NSInteger )index withArr:(NSMutableArray *)arry{
    ...
    [[AFClient shareInstance] postFile:str withArr:postOneArr progressBlock:^(NSProgress *progress) {
        ...
        if ([[responseBody valueForKey:@"status"] integerValue] == 200) {
            [uploadImagesArr addObject:[responseBody valueForKey:@"data"]];
