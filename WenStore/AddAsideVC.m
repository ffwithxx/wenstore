...
#import "AFClient.h"
#import "BGControl.h"
#define kCellName @"AddAsideCell"
#import "SearchVC.h"
#import "AsideDetailVC.h"
#import "orderCountThree.h"
@interface AddAsideVC ()<UITableViewDelegate,UITableViewDataSource,fanDataDelegate,asideDelegate,orderCountDelegate,TanDelegate> {
    NSInteger lpdt036;
...
-(void)jiaoyan {
    ...
    [self show];
        ...
        [[AFClient shareInstance] postValidateCart:postMastDict detail:detailArr uploadImages:uploadImgArr promoOrders:proOrderArr freeOrders:freeOrderArr withArr:postOneArr withUrl:@"App/Wbp3005/ValidateCart" withjsob001:@"" progressBlock:^(NSProgress *progress) {
            ...
            if ([[responseBody valueForKey:@"status"] integerValue] == 200) {
                ...
                if ([BGControl isNULLOfString:[userResponseDict valueForKey:@"code"]] ) {
                   ...
                   //
                    ...
                    AsideDetailVC *detail = [storyboard instantiateViewControllerWithIdentifier:@"AsideDetailVC"];
                     detail.detpsArr = self.detpsArr;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (tableView == self.leftTableView){
        ...
        AddAsideCell  * _CellTwo = [tableView dequeueReusableCellWithIdentifier:CellIdentifier];
        if (!_CellTwo) {
            _CellTwo = [[AddAsideCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:CellIdentifier];
        }
        ...
        AsideModel *model = arr[indexPath.row];
...
- (void)getOrderCount:(NSDecimalNumber *)count withModel:(AsideModel *)model {
    ...
    for (AsideModel *oneModel in postArr) {
...
-(void)scrapwithModel:(AsideModel *)model{
    ...
    orderView.asideModel = model;
    orderView.typeStr = @"3005";
    orderView.asideDelegate = self;
