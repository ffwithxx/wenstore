//
//  DialDetailVC.mApp/Wbp3010/Update//  WenStore
//
...
#define kCellName @"DialDetailCell"
@interface DialDetailVC ()<RemarkDelegate,UITableViewDelegate,UITableViewDataSource> {
    DialDetailCell *_cell;
    NSInteger lpdt043;
    NSInteger lpdt042;
    NSInteger lpdt036;
...
- (void)first {
    ...
    self.boruDate.text = [BGControl changeNsdate:[NSDate date]];
...
-(void)update{
    ...
    [postMast setValue:[NSString stringWithFormat:@"%@",self.boruDate.text] forKey:@"k1mf003"];
     [postMast setValue:[NSString stringWithFormat:@"%@",self.boruDate.text] forKey:@"k1mf004"];
    ...
    if (isConfirm == true) {
        urlStr = @"App/Wbp3010/UpdateAndConfirm";
    }else {
        urlStr = @"App/Wbp3010/Update";
    }
    ...
    [[AFClient shareInstance] Update:updateDict withArr:postOneArr withUrl:urlStr  progressBlock:^(NSProgress *progress) {
        ...
        if ([[responseBody valueForKey:@"status"] integerValue] == 200) {
            ...
            if ([BGControl isNULLOfString:[userResponseDict valueForKey:@"code"]] ) {
                ...
                dialOnePageVC *orderVC = [storyboard instantiateViewControllerWithIdentifier:@"dialOnePageVC"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (!_cell) {
        _cell = [[DialDetailCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:kCellName];
    }
    ...
    DialModel *model = self.dataArray[indexPath.section];
...
- (IBAction)buttonClick:(UIButton *)sender {
    if (sender.tag == 201) {
        ...
        calendarPicker.calendarBlock = ^(NSInteger day, NSInteger month, NSInteger year){
            ...
            self.boruDate.text = [NSString stringWithFormat:@"%@-%@-%@", yearStr,monthStr,dayStr];
          
            bochuDate = [BGControl stringToDate:self.boruDate.text];
        };
