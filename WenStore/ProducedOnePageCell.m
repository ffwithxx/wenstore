...
#import "BGControl.h"
@implementation ProducedOnePageCell{
    ProducedOnePageModel *oneModel;
}
...
-(void)showModel:(ProducedOnePageModel *)model {
    ...
    NSInteger lpdt043 = [[NSString stringWithFormat:@"%@",[[NSUserDefaults standardUserDefaults] valueForKey:@"3007lpdt043"] ] integerValue];
    self.nameLab.text = model.k1mf101;
    self.remarkLab.text = model.k1mf010;
    self.idStrLab.text = model.k1mf100;
    self.sumCountLab.text = [NSString stringWithFormat:@"%@%@%@",@"上报",model.k1mf303,@"件商品"];
