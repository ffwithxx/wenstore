...
#define kCellName @"AsideOnePageCell"
@interface AsideOnePageVC ()<UITableViewDelegate,UITableViewDataSource,orderDelegete>{
    AsideOnePageCell *_cell;
...
-(void)getDate {
    ...
    [self show];
    [[AFClient shareInstance] OnePage:postDict withArr:postOneArr withUrl:@"App/Wbp3005/OnePage" progressBlock:^(NSProgress *progress) {
        ...
        if ([[responseBody valueForKey:@"status"] integerValue]== 200) {
            ...
            if ([BGControl isNULLOfString:[userResponseDict valueForKey:@"code"]] ) {
                ...
                for (int i = 0; i<dataArr.count; i++) {
                    ...
                    AsideOnePageModel *model = [AsideOnePageModel new];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (!_cell) {
        _cell = [[AsideOnePageCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:kCellName];
    }
    ...
    AsideOnePageModel *model = self.dataArray[indexPath.section];
...
-(void)postoneStr:(NSString *)tagStr withModel:(AsideOnePageModel *)model {
    ...
    if ([tagStr isEqualToString:@"301"]) {
        ...
        if (model.isDisplayEditButton) {
            ...
            AddAsideVC *mainVC = [storyboard instantiateViewControllerWithIdentifier:@"AddAsideVC"];
...
-(void)PreviewWithIdStr:(NSString *)idStr{
    [self show];
    [[AFClient shareInstance] WBP3008Preview:idStr withArr:postOneArr withUrl:@"App/Wbp3005/Preview" progressBlock:^(NSProgress *progress) {
        ...
        if ([[responseBody valueForKey:@"status"] integerValue]==200) {
            ...
            if ([BGControl isNULLOfString:[userResponseDict valueForKey:@"code"]] ) {
                ...
                [self GreateWith:[responseBody valueForKey:@"data"]];
...
- (void)GreateWith:(NSDictionary *)dataDic{
    ...
    [[AFClient shareInstance] Update:postMast withArr:postOneArr withUrl:@"App/Wbp3005/UpdateAndConfirm"  progressBlock:^(NSProgress *progress) {
...
-(void)delet:(NSString *)idStr {
    [self show];
    [[AFClient shareInstance] Destroy:idStr withArr:postOneArr withUrl:@"App/Wbp3005/Destroy" progressBlock:^(NSProgress *progress) {
    ...
    [[AFClient shareInstance] WBP3008Preview:idStr withArr:postOneArr withUrl:@"App/Wbp3005/Preview" progressBlock:^(NSProgress *progress) {
        ...
        if ([responseBody[@"status"] integerValue] == 200) {
            ...
            if ([BGControl isNULLOfString:[userResponseDict valueForKey:@"code"]] ) {
                ...
                AsideVC *detail =[storboard  instantiateViewControllerWithIdentifier:@"AsideVC"];
                detail.idStr = idStr;
