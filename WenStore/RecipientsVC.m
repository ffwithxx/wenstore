...
#import "SearchVC.h"
#import "BGControl.h"
#define kCellName @"RecipientsCell"
@interface RecipientsVC ()<UITableViewDelegate,UITableViewDataSource,fanDataDelegate>{
    NSInteger lpdt036;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (tableView == self.leftTableView){
        ...
        // 通过不同标识创建cell实例
        RecipientsCell  * _twoCell = [tableView dequeueReusableCellWithIdentifier:kCellName];
        if (!_twoCell) {
            _twoCell = [[RecipientsCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:kCellName];
        }
