...
#import "AddTryEatCell.h"
#import "TryEatModel.h"
#import "AFClient.h"
#import "BGControl.h"
#define kCellName @"AddTryEatCell"
#import "SearchVC.h"
#import "TryEatDetailVC.h"
#import "orderCountThree.h"

@interface AddTryEatVC ()<UITableViewDelegate,UITableViewDataSource,fanDataDelegate,asideDelegate,orderCountDelegate,TanDelegate,tryEatDelegate> {
    NSInteger lpdt036;
...
- (void)first {
    ...
    lpdt043 = [[NSString stringWithFormat:@"%@",[[NSUserDefaults standardUserDefaults] valueForKey:@"3018lpdt043"] ] integerValue];
    lpdt036 = [[NSString stringWithFormat:@"%@",[[NSUserDefaults standardUserDefaults] valueForKey:@"3018lpdt036"] ] integerValue];
    lpdt042 = [[NSString stringWithFormat:@"%@",[[NSUserDefaults standardUserDefaults] valueForKey:@"3018lpdt042"] ] integerValue];
...
-(void)firstOne {
    ...
    if ([self.typeStr isEqualToString:@"add"]) {
        urlStr = @"App/Wbp3018/New";
    }else{
        urlStr = @"App/Wbp3018/Edit";
    }
    [self show];
            ...
            if ([BGControl isNULLOfString:[userResponseDict valueForKey:@"code"]] ) {
                ...
                for (int i = 0; i<dataArr.count; i++) {
                    ...
                    for (int j = 0; j<dictDetail.count; j++) {
                        TryEatModel *model  = [TryEatModel new];
...
-(void)jiaoyan {
    ...
    [[AFClient shareInstance] postValidateCart:mastDict detail:detailArr uploadImages:uploadImgArr promoOrders:proOrderArr freeOrders:freeOrderArr withArr:postOneArr withUrl:@"App/Wbp3018/ValidateCart" withjsob001:@"" progressBlock:^(NSProgress *progress) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (tableView == self.leftTableView){
        ...
        AddTryEatCell  * _CellTwo = [tableView dequeueReusableCellWithIdentifier:CellIdentifier];
        if (!_CellTwo) {
            _CellTwo = [[AddTryEatCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:CellIdentifier];
        }
        ...
        TryEatModel *model = arr[indexPath.row];
...
-(void) getTryEatOrderCount:(NSDecimalNumber *)count withModel:(TryEatModel *)model {
    ...
    for (TryEatModel *oneModel in postArr) {
    ...
    for (int i = 0; i <postArr.count; i++) {
        TryEatModel *modelTwo = postArr[i];
...
-(void)scrapwithModel:(TryEatModel *)model{
    ...
    orderView.tryEatModel = model;
    orderView.typeStr = @"3018";
    orderView.tryEatDelegate = self;
