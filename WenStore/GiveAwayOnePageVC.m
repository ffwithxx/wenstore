...
#define kCellName @"GiveAwayOnePageCell"
@interface GiveAwayOnePageVC ()<UITableViewDelegate,UITableViewDataSource,orderDelegete>{
    GiveAwayOnePageCell *_cell;
...
@implementation GiveAwayOnePageVC
...
-(void)getDate {
    ...
    [self show];
    [[AFClient shareInstance] OnePage:postDict withArr:postOneArr withUrl:@"App/Wbp3019/OnePage" progressBlock:^(NSProgress *progress) {
        ...
        if ([[responseBody valueForKey:@"status"] integerValue]== 200) {
            ...
            if ([BGControl isNULLOfString:[userResponseDict valueForKey:@"code"]] ) {
                ...
                for (int i = 0; i<dataArr.count; i++) {
                    ...
                    GiveAwayOnePageModel *model = [GiveAwayOnePageModel new];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (!_cell) {
        _cell = [[GiveAwayOnePageCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:kCellName];
    }
    ...
    GiveAwayOnePageModel *model = self.dataArray[indexPath.section];
...
-(void)postoneStr:(NSString *)tagStr withModel:(GiveAwayOnePageModel *)model {
    if ([tagStr isEqualToString:@"301"]) {
        ...
        if (model.isDisplayEditButton) {
            ...
            AddGiveAwayVC *mainVC = [storyboard instantiateViewControllerWithIdentifier:@"AddGiveAwayVC"];
...
-(void)PreviewWithIdStr:(NSString *)idStr{
    [self show];
    [[AFClient shareInstance] WBP3008Preview:idStr withArr:postOneArr withUrl:@"App/Wbp3019/Preview" progressBlock:^(NSProgress *progress) {
        ...
        if ([[responseBody valueForKey:@"status"] integerValue]==200) {
            ...
            if ([BGControl isNULLOfString:[userResponseDict valueForKey:@"code"]] ) {
                ...
                UIStoryboard *storyboard = [UIStoryboard storyboardWithName:@"WendianTwo" bundle:nil];
                NSDictionary *dict = [responseBody valueForKey:@"data"];
                GiveAwayDetailVC *detail = [storyboard instantiateViewControllerWithIdentifier:@"GiveAwayDetailVC"];
                detail.idStr = idStr;
                detail.typeStr = @"2";
...
- (void)GreateWith:(NSDictionary *)dataDic{
    ...
    [masterDict setValue:[dict valueForKey:@"k1mf008"] forKey:@"k1mf008"];
    [masterDict setValue:[dict valueForKey:@"k1mf010"] forKey:@"k1mf010"];
    [masterDict setValue:[dict valueForKey:@"k1mf011"] forKey:@"k1mf011"];
    [masterDict setValue:[dict valueForKey:@"k1mf100"] forKey:@"k1mf100"];
    [masterDict setValue:[dict valueForKey:@"k1mf109"] forKey:@"k1mf109"];
    [masterDict setValue:[dict valueForKey:@"k1mf201"] forKey:@"k1mf201"];
    [masterDict setValue:[dict valueForKey:@"k1mf203"] forKey:@"k1mf203"];
    [masterDict setValue:[dict valueForKey:@"k1mf301"] forKey:@"k1mf301"];
    ...
    [masterDict setValue:[dict valueForKey:@"k1mf998"] forKey:@"k1mf998"];
    [masterDict setValue:[dict valueForKey:@"k1mf999"] forKey:@"k1mf999"];
    
    NSMutableDictionary *postMast = [NSMutableDictionary dictionaryWithObjectsAndKeys:masterDict,@"master",postArr,@"detail",nil];
    [[AFClient shareInstance] Update:postMast withArr:postOneArr withUrl:@"App/Wbp3019/UpdateAndConfirm"  progressBlock:^(NSProgress *progress) {
...
-(void)delet:(NSString *)idStr {
    [self show];
    [[AFClient shareInstance] Destroy:idStr withArr:postOneArr withUrl:@"App/Wbp3019/Destroy" progressBlock:^(NSProgress *progress) {
...
-(void)getPreviewWithIdStr:(NSString *)idStr {
    ...
    [[AFClient shareInstance] WBP3008Preview:idStr withArr:postOneArr withUrl:@"App/Wbp3019/Preview" progressBlock:^(NSProgress *progress) {
        ...
        if ([responseBody[@"status"] integerValue] == 200) {
            NSDictionary *userResponseDict = [[responseBody valueForKey:@"data"] valueForKey:@"userResponse"];
            if ([BGControl isNULLOfString:[userResponseDict valueForKey:@"code"]] ) {
                UIStoryboard *storboard = [UIStoryboard storyboardWithName:@"WendianTwo" bundle:nil];
                GiveAwayVC *detail =[storboard  instantiateViewControllerWithIdentifier:@"GiveAwayVC"];
                detail.idStr = idStr;
                detail.dataDict = [responseBody valueForKey:@"data"];
                [self.navigationController pushViewController:detail animated:YES];
