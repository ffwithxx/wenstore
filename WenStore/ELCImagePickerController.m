...
- (BOOL)shouldSelectAsset:(ELCAsset *)asset previousCount:(NSUInteger)previousCount
{
    ...
    if (!shouldSelect) {
        NSString *title = [NSString stringWithFormat:NSLocalizedString(Localized(@"您最多只能上传9张图片"), nil), self.maximumImagesCount];
      //  NSString *message = [NSString stringWithFormat:NSLocalizedString(@"You can only send %d photos at a time.", nil), self.maximumImagesCount];
        [[[UIAlertView alloc] initWithTitle:title
                          ...
                          otherButtonTitles:NSLocalizedString(Localized(@"确认"), nil), nil] show];
    }
    return shouldSelect;
}
