...
#define kCellName @"ProducedDetailCell"
@interface ProducedDetailVC ()<RemarkDelegate,UITableViewDelegate,UITableViewDataSource> {
    ProducedDetailCell *_cell;
    NSInteger lpdt043;
    NSInteger lpdt042;
    NSInteger lpdt036;
...
@implementation ProducedDetailVC
...
- (void)first {
    ...
    self.prioducedDateField.text = [BGControl changeNsdate:[NSDate date]];
    
    prioducedDate = [NSDate date];
...
-(void)update{
    ...
    [postMast setValue:[NSString stringWithFormat:@"%@",self.prioducedDateField.text] forKey:@"k1mf003"];
    [postMast setValue:[NSString stringWithFormat:@"%@",self.prioducedDateField.text] forKey:@"k1mf004"];
    ...
    if (isConfirm == true) {
        urlStr = @"App/Wbp3007/UpdateAndConfirm";
    }else {
        urlStr = @"App/Wbp3007/Update";
    }
    }
    ...
    [[AFClient shareInstance] Update:updateDict withArr:postOneArr withUrl:urlStr  progressBlock:^(NSProgress *progress) {
        ...
        if ([[responseBody valueForKey:@"status"] integerValue] == 200) {
            ...
            if ([BGControl isNULLOfString:[userResponseDict valueForKey:@"code"]] ) {
                ...
                ProducedOnePaggeVC *orderVC = [storyboard instantiateViewControllerWithIdentifier:@"ProducedOnePaggeVC"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (!_cell) {
        _cell = [[ProducedDetailCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:kCellName];
    }
    ...
    ProducedModel *model = self.dataArray[indexPath.section];
...
- (IBAction)buttonClick:(UIButton *)sender {
    if (sender.tag == 201) {
        ...
        calendarPicker.calendarBlock = ^(NSInteger day, NSInteger month, NSInteger year){
            ...
            self.prioducedDateField.text = [NSString stringWithFormat:@"%@-%@-%@", yearStr,monthStr,dayStr];
           
            prioducedDate = [BGControl stringToDate:self.prioducedDateField.text];
        };
