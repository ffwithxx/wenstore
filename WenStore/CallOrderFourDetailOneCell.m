...
@implementation CallOrderFourDetailOneCell {
    EditModel *oneModel;
...
//    self.headImg.image = [UIImage imageNamed:@"icon_moren(1).png"];;
    ...
    [self.minuBth addTarget:self action:@selector(buttonClick:) forControlEvents:UIControlEventTouchUpInside];
    ...
    [self.jiaBth addTarget:self action:@selector(buttonClick:) forControlEvents:UIControlEventTouchUpInside];
...
- (void)showModel:(EditModel *)model{
    ...
    self.resonTextFile.placeholder = @"请选择退回原因";
    self.resonSmTextFile.placeholder = @"请选择退回原因说明";
    self.resonSmTextFile.text = model.k1dt503;
    self.resonTextFile.text = model.k1dt502;
    CGFloat  onwidth = self.contentView.frame.size.width;
...
- (void)buttonClick:(UIButton *)bth {
    if (bth.tag == 1003 ) {
        if (_inputDelegate && [_inputDelegate respondsToSelector:@selector(backWithModel:withTag:)]) {
            [_inputDelegate backWithModel:oneModel withTag:bth.tag];
        }
    }else{
        ...
        if (_oneDelegate && [_oneDelegate respondsToSelector:@selector(backWithModel:withTag:withCount:)]) {
            [_oneDelegate backWithModel:oneModel withTag:bth.tag withCount:numCount];
        }
