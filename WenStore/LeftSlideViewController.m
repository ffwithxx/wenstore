                     ...
                     andMainView:(UIViewController *)mainVC
{
    if(self = [super init]){
        ...
        viewone.frame = self.leftVC.view.bounds;
        viewone.backgroundColor = [UIColor blackColor];
        viewone.alpha = 0.5;
        self.contentView = viewone;
        [self.leftVC.view addSubview:viewone];
        
        //获取左侧tableview
        for (UIView *obj in self.leftVC.view.subviews) {
            if ([obj isKindOfClass:[UIView class]]) {
                if (obj.tag == 1002) {
                      self.leftTableview = (UIView *)obj;
                }
...
- (void)openLeftView;
{
   ...
   view = [[UIView alloc] initWithFrame:CGRectMake(0, 0, self.mainVC.view.frame.size.width, self.mainVC.view.frame.size.height)];
    view.alpha = 0.3;
    view.tag = 1001;
