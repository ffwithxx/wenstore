...
#define kCellName @"ThreeDetailCell"

@interface ThreeDetailVC ()<RemarkDelegate,UITableViewDelegate,UITableViewDataSource,UIActionSheetDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate,UITextFieldDelegate,UITextViewDelegate,ELCImagePickerControllerDelegate,postArrDelegate,UIScrollViewDelegate> {
    ThreeDetailCell *_cell;
...
-(void)first {
    ...
    self.sumCountLab.text = [NSString stringWithFormat:@"%@%@%@",@"收货",countStr,@"件商品"];
     NSArray *time = [[BGControl dateToDateStringTwo:[masterDict valueForKey:@"k1mf003"]] componentsSeparatedByString:@" "];
    self.jisohuoDateFile.text = time[0];
    NSArray *timeOne = [[BGControl dateToDateStringTwo:[masterDict valueForKey:@"k1mf004"]] componentsSeparatedByString:@" "];
     self.peisongDateFile.text = timeOne[0];
     jiaohuoDate = [masterDict valueForKey:@"k1mf003"];
     peiDate = [masterDict valueForKey:@"k1mf004"];
    ...
    for (int i = 0; i<oneArr.count; i++) {
        ...
        NSDecimalNumber *one = [[NSDecimalNumber decimalNumberWithString:[BGControl notRounding:model.k1dt201 afterPoint:lpdt042]] decimalNumberByMultiplyingBy:[NSDecimalNumber decimalNumberWithString:[BGControl notRounding:model.k1dt103 afterPoint:lpdt036]]];
...
- (IBAction)buttonClick:(UIButton *)sender {
    ...
    if (sender.tag == 201) {
        ...
        calendarPicker.calendarBlock = ^(NSInteger day, NSInteger month, NSInteger year){
            ...
            self.jisohuoDateFile.text = [NSString stringWithFormat:@"%@-%@-%@", yearStr,monthStr,dayStr];
           
            jiaohuoDate = [BGControl stringToDate:self.jisohuoDateFile.text];
        ...
        calendarPicker.calendarBlock = ^(NSInteger day, NSInteger month, NSInteger year){
            ...
            self.peisongDateFile.text = [NSString stringWithFormat:@"%@-%@-%@", yearStr,monthStr,dayStr];
           
            peiDate = [BGControl stringToDate:self.peisongDateFile.text];
...
-(void)update{
    ...
    [postMast setValue:self.jisohuoDateFile.text forKey:@"k1mf003"];
//    [postMast setValue:[NSString stringWithFormat:@"%@",peiDate] forKey:@"k1mf004"];
    ...
    NSString *urlStr;
    if (isConfirm == true) {
         urlStr = @"App/Wbp3008/UpdateAndConfirm";
    }else{
          urlStr = @"App/Wbp3008/Update";
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (!_cell) {
        _cell = [[ThreeDetailCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:kCellName];
    }
